...
def drawPoint2D(x, y, color, camera, fpu=False):
    glPushMatrix()
    ...
    if not fpu:
        glVertex3f(x1 * camera.orient[0][0] + y1 * camera.orient[1][0] + (-1000) * camera.orient[2][0],
                   x1 * camera.orient[0][1] + y1 * camera.orient[1][1] + (-1000) * camera.orient[2][1],
                   x1 * camera.orient[0][2] + y1 * camera.orient[1][2] + (-1000) * camera.orient[2][2])
    else:
        x1 = -x1
        glVertex3f(x1 * camera.orient[0][0] + y1 * camera.orient[1][0] + (1000) * camera.orient[2][0],
                   x1 * camera.orient[0][1] + y1 * camera.orient[1][1] + (1000) * camera.orient[2][1],
...
def drawLine2D(x1, y1, x2, y2, color, camera, fpu=False):
    glPushMatrix()
    glTranslate(-camera.pos[0],
                -camera.pos[1],
                -camera.pos[2])
    ...
    if not fpu:
        ...
        glVertex3f(x2 * camera.orient[0][0] + y2 * camera.orient[1][0] + (-1000) * camera.orient[2][0],
                   x2 * camera.orient[0][1] + y2 * camera.orient[1][1] + (-1000) * camera.orient[2][1],
                   x2 * camera.orient[0][2] + y2 * camera.orient[1][2] + (-1000) * camera.orient[2][2])
    else:
        x1 = -x1
        x2 = -x2
        glVertex3f(x1 * camera.orient[0][0] + y1 * camera.orient[1][0] + (1000) * camera.orient[2][0],
...
def drawRectangle2D(x1, y1, x2, y2, color, camera, fpu=False):
    drawLine2D(x1, y1, x2, y1, color, camera, fpu)
    drawLine2D(x1, y1, x1, y2, color, camera, fpu)
    drawLine2D(x2, y1, x2, y2, color, camera, fpu)
    drawLine2D(x1, y2, x2, y2, color, camera, fpu)
...
def render_numbers(numstring, color, start_pt, cam, font_size=0.5, fpu=False):
    global numbers
    ...
    for char in numstring:
        if not char == ".":
            ...
            for line in lines:
                ...
                drawLine2D(x1, y1, x2, y2, color, cam, fpu)

        else:
            x = draw_start_x
            y = start_pt[1]
            drawPoint2D(x, y, color, cam, fpu)

        draw_start_x += font_size + spacing

def render_AN(render_string, color, start_pt, cam, font_size=0.1, fpu=False):
