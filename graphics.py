...
def drawBoostTrails(m):
    if len(m.boost_trail) < 2:
        return
    
    glPushMatrix()
    glColor(1, 0.8, 0)
    glBegin(GL_LINES)
    for p in m.boost_trail:
        glVertex3f(p[0], p[1], p[2])
    glEnd()
    glPopMatrix()

def drawScene(cam, flatland, bodies, scenery_objects):
    drawFlatland(cam, flatland)
    for o in scenery_objects:
        drawModel(o.model, o.pos, np.eye(3), 1, Color(0, 1, 1))

    for b in bodies:
        if np.linalg.norm(b.pos - cam.pos) < 100e3:
